...
class Switch(StpSwitch):
    ...
    def send_initial_messages(self):
        ...
        for neighbor in self.links:
            message = Message(self.root, self.distance, self.switchID, neighbor, False)
    ...
    def process_message(self, message):
        ...
        if not hasUpdate:
            if message.pathThrough and message.origin not in self.activeLinks:
                ...
                if len(self.activeLinks) > 1 and message.origin != self.switchThrough:
        ...
        if hasUpdate:
            for neighbor in self.links:
                if neighbor == self.switchThrough:
                    ...
                    pathTrough = False
                message = Message(self.root, self.distance, self.switchID, neighbor, pathTrough)
                self.send_message(message)
        return
    ...
    def generate_logstring(self):
        ...
        outputString = ''
        startLine = True
        for neighbor in sorted(self.activeLinks):
            if startLine:
                outputString = outputString + '{} - {}'.format(self.switchID, neighbor)
            else:
                outputString = outputString + ', {} - {}'.format(self.switchID, neighbor)
            startLine = False
